...
int ring_buffer_init(ring_buffer_t *rb, size_t capacity, size_t elt_size) {
    ...
    // Make sure that (cap + 1) * sz ≤ SIZE_MAX so that there's no overflow.
    // (cap + 1) * sz ≤ SIZE_MAX ⇔ cap + 1 ≤ SIZE_MAX / sz
    //                           ⇔ cap     < SIZE_MAX / sz
    ...
    // Reserve one empty slot for empty/full checking.
    rb->capacity = capacity + 1;
    rb->elt_size = elt_size;

    if (!(rb->data = malloc(rb->capacity * rb->elt_size))) {
...
int _ring_buffer_resize(ring_buffer_t *rb, size_t capacity) {
    ...
    uint8_t *tmp = realloc(rb->data, (capacity + 1) * rb->elt_size);
    if (!tmp) {
...
int ring_buffer_resize(ring_buffer_t *rb, size_t capacity) {
    ...
    bool should_reset = capacity + 1 < rb->capacity;
...
int ring_buffer_push(ring_buffer_t *rb, void *value) {
    ...
    if ((tail + 1) % rb->capacity == head) {
        DBG("ring_buffer_push: buffer full\n");
        return -1;
    }

    memcpy(rb->data + (tail * rb->elt_size), value, rb->elt_size);
    atomic_store_explicit(&rb->tail, (tail + 1) % rb->capacity,
...
int ring_buffer_pop(ring_buffer_t *rb, void *out) {
    ...
    memcpy(out, rb->data + (head * rb->elt_size), rb->elt_size);
    atomic_store_explicit(&rb->head, (head + 1) % rb->capacity,
                          memory_order_release);
    return 0;
}
